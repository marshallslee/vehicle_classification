...
def main():
    ...
    train_loader, val_loader, test_loader, class_to_idx = get_loaders(
        ...
        args.auto_split, args.class_names, args.use_weighted_sampler
    )
    num_classes = len(class_to_idx)
    model = build_model(args.arch, num_classes=num_classes, dropout=args.dropout,
    ...
    best_val = float('inf'); best_path = os.path.join(save_dir, "best.ckpt")
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for xb, yb in pbar:
            ...
            logits = model(xb)
            loss = mixup_criterion(criterion, logits, ya, yb2, lam) if args.mixup else criterion(logits, yb)
        ...
        if val_loss < best_val:
            best_val = val_loss
            torch.save({"state_dict": model.state_dict(), "arch": args.arch, "class_to_idx": class_to_idx}, best_path)
    ...
    print("Test Accuracy:", acc); print(rep)

    json.dump({"val_best_loss": best_val, "test_accuracy": acc, "report": rep},

...
def safe_makedirs(p: Path): p.mkdir(parents=True, exist_ok=True)

def copy_or_crop(src_img: Path, dst_img: Path,
...
def stanford_bbox_for_image(img_path: Path, stanford_root: Path) -> Optional[Tuple[int,int,int,int]]:
    try:
        rel = img_path.relative_to(stanford_root / "image")
        label_txt = stanford_root / "label" / rel.with_suffix(".txt")
        if not label_txt.exists(): return None
        lines = [l.strip() for l in open(label_txt, "r", encoding="utf-8", errors="ignore").read().splitlines() if l.strip()]
        if len(lines) >= 3:
...
def prepare_from_stanford(stanford_root: str, out: str, img_size: int, crop_bbox: bool):
    ...
    def save_split(imgs: List[Path], split: str):
        ...
        for p in imgs:
            ...
            copy_or_crop(p, dst, bbox, img_size)
...
def prepare_from_compcars(compcars_root: str, out: str, img_size: int, crop_bbox: bool):
    ...
    def copy_dir(src_dir: Path, split: str, ann):
        dst_dir = out / split / "car"; safe_makedirs(dst_dir)
        for p in sorted(src_dir.glob("*.jpg")):
            dst = dst_dir / p.name
            bbox = ann.get(p.name) if (ann and p.name in ann) else None
            copy_or_crop(p, dst, bbox, img_size)

    if train_dir.exists(): copy_dir(train_dir, "train", train_ann)
    if test_dir.exists():  copy_dir(test_dir,  "test",  test_ann)
...
def main():
    ...
    if args.stanford_root:
        ...
        prepare_from_stanford(args.stanford_root, args.out, args.img_size, args.crop_bbox)

    if args.compcars_root:
        print("[CompCars] preparing...")
        prepare_from_compcars(args.compcars_root, args.out, args.img_size, args.crop_bbox)

    if args.truck_dir:
        print("[Truck] adding...")
        add_single_class(args.truck_dir, args.out, cls_name="truck", img_size=args.img_size)
    if args.bus_dir:
        print("[Bus] adding...")
        add_single_class(args.bus_dir, args.out, cls_name="bus", img_size=args.img_size)
